import { Mesh, Scene, Vector3, ParticleSystem, Texture, Color4, VertexBuffer } from "@babylonjs/core";
...
export class Artifice
{
	...
	constructor(scene: Scene)
	{
		...
		this.isFired = false;
		this.timer1 = 0;
		this.textureFirework = "projects/scene/textures/flare.png";
	}
	
	shoot(posX = 0, posY = -20, posZ = 0)
	{
		let startSphere = Mesh.CreateSphere("Shoot", 4, 1, this.scene);
		...
		let updateFunction = function(particles) {
			for (let index = 0; index < particles.length; index++) {
				...
				if (particle.age >= particle.lifeTime) {
					...
					particle.direction.scaleToRef(particleSystem.updateSpeed, particleSystem.direction1);
					particle.position.addInPlace(particleSystem.direction1);
					particleSystem.gravity.scaleToRef(particleSystem.updateSpeed, particleSystem.gravity);
					particle.direction.addInPlace(particleSystem.gravity);
				}
			}
		};
    ...
    firework(shootX: number, shootZ: number)
    { 
        let fountain = Mesh.CreateSphere("explosion", 4, 1, this.scene);
        fountain.isVisible = false;
        fountain.position.x = shootX;
        fountain.position.y = this.getRandomBetween(30, 40);
        fountain.position.z = shootZ;
		...
		let updateFunction = function(particles) {
			for (let index = 0; index < particles.length; index++) {
				...
				if (particle.age >= particle.lifeTime) {
					...
					particle.direction.scaleToRef(particleSystem1.updateSpeed, particleSystem1.direction1);
					particle.position.addInPlace(particleSystem1.direction1);
					particleSystem1.gravity.scaleToRef(particleSystem1.updateSpeed, particleSystem1.gravity);
					particle.direction.addInPlace(particleSystem1.gravity);
				}
			}
		};
